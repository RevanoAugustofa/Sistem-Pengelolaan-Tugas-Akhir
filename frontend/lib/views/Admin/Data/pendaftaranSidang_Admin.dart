...
class _PendaftaranSidangAdminPageState extends State<PendaftaranSidangAdminPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        if (controller.isLoadingPendaftaranSidang.value) {
          return const Center(child: CircularProgressIndicator());
        }

        var filteredMahasiswa = controller.listPendaftaranSidang.where((mhs) {
          final query = searchQuery.toLowerCase();
          return (mhs["mahasiswa"]["nama"]?.toString().toLowerCase().contains(query) ?? false) ||
                 (mhs["mahasiswa"]["npm"]?.toString().toLowerCase().contains(query) ?? false) ||
        ...
        List<dynamic> displayedUsers = filteredMahasiswa.isEmpty ? [] : filteredMahasiswa.sublist(startIndex, endIndex);

        return RefreshIndicator(
          onRefresh: () async => controller.fetchPendaftaranSidang(),
                      ...
                      if (filteredMahasiswa.isEmpty)
                        const Center(
                          child: Padding(
                            ...
                            child: Text("Belum ada data pendaftaran sidang", style: TextStyle(color: Colors.grey)),
                          ),
                        )
                      else
                              ...
                              rows: displayedUsers.asMap().entries.map((entry) {
                                ...
                                return DataRow(
                                  ...
                                  cells: [
                                    ...
                                    DataCell(Text(user["mahasiswa"]["npm"].toString())),
                                    DataCell(Text(user["mahasiswa"]["nama"])),
                                    DataCell(Text(user["prodi"])),
                                    DataCell(Text(user["mahasiswa"]["tahun_ajar"])),
                                  ],
                                );
                              }).toList(),
                            ),
