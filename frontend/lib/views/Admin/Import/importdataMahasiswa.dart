...
class _ImportDataMahasiswaPageState extends State<ImportDataMahasiswaPage> {
  ...
  Future<void> _downloadTemplate() async {
    ...
    sheetObject.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: 0)).value = TextCellValue("ID Prodi");
    sheetObject.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: 0)).value = TextCellValue("ID Tahun Ajar");
    ...
    sheetObject.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: 1)).value = TextCellValue("1");
    sheetObject.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: 1)).value = TextCellValue("1");
    ...
    try {
      ...
      for (var table in excel.tables.keys) {
        ...
        for (int i = 1; i < rows.length; i++) {
          ...
          int? idProdi = (row.length > 4) ? int.tryParse(row[4]?.value?.toString() ?? "") : null;
          int? idTahunAjar = (row.length > 5) ? int.tryParse(row[5]?.value?.toString() ?? "") : null;
