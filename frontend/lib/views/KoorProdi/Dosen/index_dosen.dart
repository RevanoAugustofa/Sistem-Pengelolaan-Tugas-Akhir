...
class IndexDosenPage extends StatefulWidget {
  const IndexDosenPage({super.key});

  @override
  State<IndexDosenPage> createState() => _IndexDosenPageState();
}

class _IndexDosenPageState extends State<IndexDosenPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        var filteredDosen = controller.listDosenManajemen.where((dsn) {
          final query = searchQuery.toLowerCase();
          return (dsn.namaDosen?.toLowerCase().contains(query) ?? false) ||
                 (dsn.nip?.toLowerCase().contains(query) ?? false);
        }).toList();
        ...
        return RefreshIndicator(
          onRefresh: () async => controller.fetchDosenManajemen(),
                            ...
                            rows: List.generate(displayedDosen.length, (index) {
                              ...
                              return DataRow(
                                ...
                                cells: [
                                  ...
                                  DataCell(Text(dsn.nip ?? "-")),
                                  DataCell(Text(dsn.namaDosen ?? "-")),
                                  DataCell(Text(dsn.jabatan ?? "-")),
                                  DataCell(Row(
                                    children: [
                                      IconButton(
                                        icon: const Icon(Icons.edit, color: Colors.blue, size: 20),
                                        onPressed: () => Get.to(() => EditDosenPage(dosen: dsn)),
                                      ),
                                      IconButton(
                                        icon: const Icon(Icons.delete, color: Colors.red, size: 20),
                                        onPressed: () => _confirmDelete(dsn),
                                      ),
                                    ],
                                  )),
                                ],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "Apakah Anda yakin ingin menghapus dosen ${dsn.namaDosen}? Tindakan ini tidak dapat dibatalkan.",
