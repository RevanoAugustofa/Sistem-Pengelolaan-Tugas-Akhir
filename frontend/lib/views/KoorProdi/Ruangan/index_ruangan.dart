...
import 'create_ruangan.dart';
import 'edit_ruangan.dart';

class IndexRuanganPage extends StatefulWidget {
  const IndexRuanganPage({super.key});

  @override
  State<IndexRuanganPage> createState() => _IndexRuanganPageState();
}

class _IndexRuanganPageState extends State<IndexRuanganPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        if (endIndex > filteredRuangan.length) endIndex = filteredRuangan.length;
        if (startIndex >= filteredRuangan.length && filteredRuangan.isNotEmpty) {
           _currentPage = (filteredRuangan.length / _rowsPerPage).floor();
           startIndex = _currentPage * _rowsPerPage;
           endIndex = filteredRuangan.length;
        }
        
        var displayedRuangan = filteredRuangan.isEmpty ? [] : filteredRuangan.sublist(startIndex, endIndex);

        return RefreshIndicator(
          onRefresh: () async => controller.fetchRuangan(),
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () => Get.to(() => const CreateRuanganPage()),
                        icon: const Icon(Icons.post_add, color: Colors.white, size: 20),
                        label: const Text("Tambah Ruangan", style: TextStyle(color: Colors.white, fontSize: 14)),
                      ...
                      SingleChildScrollView(
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            ...
                            rows: List.generate(displayedRuangan.length, (index) {
                              var ruangan = displayedRuangan[index];
                              return DataRow(
                                ...
                                cells: [
                                  ...
                                  DataCell(Text(ruangan.gedung ?? "-")),
                                  DataCell(Row(
                                    children: [
                                      IconButton(
                                        icon: const Icon(Icons.edit, color: Colors.blue, size: 20),
                                        onPressed: () => Get.to(() => EditRuanganPage(ruangan: ruangan)),
                                      ),
                                      IconButton(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                InkWell(
                                  onTap: endIndex < filteredRuangan.length ? () => setState(() => _currentPage++) : null,
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                                    decoration: BoxDecoration(
                                      border: Border.all(color: Colors.grey.shade300),
                                      ...
                                      color: endIndex < filteredRuangan.length ? Colors.white : Colors.grey.shade100,
                                    ),
                                    child: Text(
                                      " > ",
                                      style: TextStyle(
                                        color: endIndex < filteredRuangan.length ? const Color(0xFF4FA5FF) : Colors.grey,
                                        fontWeight: FontWeight.bold,
                                      ),
                                    ),
                                  ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "Apakah Anda yakin ingin menghapus ruangan ${ruangan.namaRuangan}? Tindakan ini tidak dapat dibatalkan.",
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () {
                        controller.deleteRuangan(ruangan.id!);
                        Get.back();
                      },
                      style: ElevatedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(vertical: 15),
