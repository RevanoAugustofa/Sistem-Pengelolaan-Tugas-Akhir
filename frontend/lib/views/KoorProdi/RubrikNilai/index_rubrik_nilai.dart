...
import 'create_rubrik_nilai.dart';
import 'edit_rubrik_nilai.dart';

class IndexRubrikNilaiPage extends StatefulWidget {
  const IndexRubrikNilaiPage({super.key});

  @override
  State<IndexRubrikNilaiPage> createState() => _IndexRubrikNilaiPageState();
}

class _IndexRubrikNilaiPageState extends State<IndexRubrikNilaiPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        var displayedRubrik = filteredRubrik.isEmpty ? [] : filteredRubrik.sublist(startIndex, endIndex);

        return RefreshIndicator(
          onRefresh: () async => controller.fetchRubrikNilai(),
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () => Get.to(() => const CreateRubrikNilaiPage()),
                        icon: const Icon(Icons.post_add, color: Colors.white, size: 20),
                        label: const Text("Tambah Rubrik", style: TextStyle(color: Colors.white, fontSize: 14)),
                      ...
                      SingleChildScrollView(
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            ...
                            rows: List.generate(displayedRubrik.length, (index) {
                              var rubrik = displayedRubrik[index];
                              return DataRow(
                                  ...
                                  DataCell(Row(
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: () => Get.to(() => EditRubrikNilaiPage(rubrik: rubrik)),
                                      ),
                                      IconButton(
                                        icon: const Icon(Icons.delete, color: Colors.red, size: 20),
                                        onPressed: () => _confirmDelete(rubrik),
                                      ),
                                    ],
                                  )),
                                ],
  ...
  void _confirmDelete(RubrikNilai rubrik) {
    Get.dialog(
      Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "Apakah Anda yakin ingin menghapus rubrik ini? Tindakan ini tidak dapat dibatalkan.",
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () {
                        controller.deleteRubrikNilai(rubrik.id!);
                        Get.back();
                      },
                      style: ElevatedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(vertical: 15),
